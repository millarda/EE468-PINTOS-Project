...
syscall_handler (struct intr_frame *f)
{
  ...
  case SYS_WRITE:
    {
      if(is_valid_ptr((void*)(esp+1) && is_valid_ptr((void*)(esp+2) && is_valid_ptr((void*)(esp+3)))))
      {
        if(is_valid_ptr((void*)(*(esp+2)) && is_valid_ptr((void*)(*(esp+2+(*esp+3)-1)))))
          f->eax = (uint32_t) sys_write(*(esp+1), *(esp+2), *(esp+3));
        else{
          exit(-1);
        }
      }else{
...
struct file_descriptor * retrieve_file(int fd){
  ...
  struct file_descriptor *fd_struct;
  for(list_element = list_head(&open_files); list_element != list_tail(&open_files);
